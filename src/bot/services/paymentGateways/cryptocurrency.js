...
const processCryptoPayment = async (userId, amount, currency) => {
  try {
    ...
    const response = await axios.post(
      "https://api.crypto-payment-gateway.com/pay",
      {
        userId,
        amount,
        currency,
    ...
    if (response.data.success) {
      console.log(
        chalk.green(
          `Payment of ${amount} ${currency} successful for user ${userId}.`
        )
      );
      // Notify admins about the successful payment
      notifyAdmins(
        `💰 User ${userId} has made a successful payment of ${amount} ${currency}.`
      );
...
const checkPaymentStatus = async (transactionId) => {
  try {
    const response = await axios.get(
      `https://api.crypto-payment-gateway.com/payment-status/${transactionId}`
    );

    if (response.data.success) {
      console.log(
        chalk.green(
          `Payment status for transaction ${transactionId}: ${response.data.status}`
        )
      );
      return response.data.status; // Return the payment status
    } else {
      console.error(
        chalk.red(`Failed to retrieve payment status: ${response.data.message}`)
      );
